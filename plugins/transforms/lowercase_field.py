...
class LowercaseFieldOptions(BaseModel):
    source_key: str
    field: str
...
class LowercaseFieldPlugin(NodePlugin):
    name = "lowercase_field"
    category = "normalize"
    option_model = LowercaseFieldOptions
    ...
    def process(self, inputs: dict, context, source_key: str, field: str) -> pd.DataFrame:
        ...
        df[field] = df[field].apply(lambda x: x.lower() if isinstance(x, str) else x)
        return df
